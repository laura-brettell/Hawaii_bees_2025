...
```{r}
# 1. Bin the data and calculate mean coverage per chromosome per bin:

library(dplyr)

bin_width <- 200  # Set based on your desired resolution
...
# 2. For each bin, pick the chromosome with higher mean coverage:

bin_dominance <- bin_summary %>%
  group_by(bam, bin) %>%
  slice_max(mean_cov, n = 1, with_ties = FALSE) %>%  # pick the dominant chromosome
  ungroup()


# 3. Identify switch points in dominance (between bins):
...
#  4. Add vertical lines to your original plot
...
#ggsave("13Jun_Big_Island_DWV_coverage_label.pdf", plot = plot_BI_2025_label, width = 7, height = 4)

```

BI 2025 plots - old, without recomb lines
...
```{r}
# for steps 1-3, just overwriting objects fromt he original BI ones, then saving the plot and label df separately for saving
...
#ggsave("13Jun_Oahu_DWV_coverage_label.pdf", plot = plot_O_2025_label, width = 12, height = 12)
...
#ggsave("13Jun_oahu_2025_cov_plots_v2.pdf", plot = plot_o_2025a, width = 12, height = 10 )
...
#ggsave("13Jun_BI_2025_cov_plots_v2.pdf", plot = plot_BI_2025a, width = 12, height = 6 )
...
Now combine breakpoint dfs into one so I can make schemiatic
