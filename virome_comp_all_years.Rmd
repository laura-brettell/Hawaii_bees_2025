...
# for now make a colour list that will cover both naming conventions and all the LSVs (even though 3 are missing from the short read database)
...
there's definitley a better way of doing this, but it works
...
plot rel abund and proportion of bee viruses together - don't need this

```{r}

#plot2a + plot2b + plot_layout(ncol = 2)

# plots showing data for islands with varroa
...
#comb_plot_cut_V <- plot_virome_2009_cut_V +
...
# still need to change widths manually, but keep for now
...
#EEEEEEFF
#"
#comb_plot_cut_N <-  plot_virome_2012_cut_N +
...
#ggsave("31Mar_old_data_combo_plot_cutoffs_Mimo_removed.pdf", plot = comb_plot_cut, width = 15, height = 12)
...
# it look sas thought he amount of BQCV is below the cutoff threshold
# Compute relative abundance per island, ensuring it sums to 1
...
add the pies to the plots - dont need this here
...
#  plot_virome_2016_cut_V +
#  proportion_plot_2016_V +
#  plot_layout(design = layout3, guides = "collect") &  # Collects legend
#  theme(legend.position = "bottom")  # Places it below
...
#  plot_virome_2016_cut_N +
#  proportion_plot_2016_N +
#  plot_layout(design = layout4, guides = "collect") &  # Collects legend
#  theme(legend.position = "bottom")  # Places it below
...
colours for the new data - check the LSVs are the same as with the short read data. maybe edit this df in R to give viruses the same names.
...
now the proportion of total reads that belong to bee viruses, for addiitonal bar plot.

- make sure this is the proportion of total reads that are viral (not just bee viruses??? - maybe keep bee viruses, otherwise phages make up a big rproportion??) for each sample
...
#pie_2012_N_fixed   <- wrap_elements(pie_2012_N)
#pie_2015_N_fixed   <- wrap_elements(pie_2015_N)
#pie_2016_N_fixed   <- wrap_elements(pie_2016_N)
#Kauai25_pie_fixed  <- wrap_elements(Kauai25_pie)
...
then decide whether to make these into one megaplot
